...
test('events older than 31 days are hidden and pruned on startup; history is paginated', async () => {
  ...
  assert.equal(second.events[0].message, 'Event 41');
  const last = await events.list({ page: 6 });
  assert.equal(last.events.length, 3);
  assert.equal(last.events[2].message, 'Retained for the extra day');
  const restarted = new Events({ sqlite, version: '1.0.0' });
  instances.push(restarted);
  await restarted.ready;
...
test('event sorting applies to the complete history before pagination', async () => {
  ...
  assert.equal(ascending.events[9].message, 'event 09');
  assert.equal(second.events[0].message, 'Event 10');
  assert.equal(second.events[9].message, 'event 19');
