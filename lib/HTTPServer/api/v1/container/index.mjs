...
export default express()
  ...
  // createHostDirectory
  .post('/paths', async (req, res) => {
    res.status(201).json(await Docker.createHostDirectory({
  ...
  // listContainers
  .get('/', async (req, res) => {
    const [containers, currentContainer] = await Promise.all([
    ...
    res.json(containers.map(container => ({
      ...
      protected: container.Id === currentContainer?.Id,
      deletable: !container.Labels?.['containarr.app.id']
  ...
  .get('/:containerId', async (req, res) => {
    ...
    const [metadata, currentContainer] = await Promise.all([
      container.inspect(),
      Docker.getCurrentContainerMetadata().catch(() => null),
    ]);
    ...
    res.json({
      ...
      protected: metadata.Id === currentContainer?.Id,
      deletable: !metadata.Config.Labels?.['containarr.app.id']
        && metadata.Id !== currentContainer?.Id,
  ...
  .post('/:containerId/start', async (req, res) => {
    const container = await getMutableContainer(req.params.containerId);
  ...
  .delete('/:containerId', async (req, res) => {
    const { containerId } = req.params;
    await getMutableContainer(containerId);
    await Docker.deleteContainer({ containerId });
    res.status(204).send();
  });

async function getMutableContainer(containerId) {
  const container = await Docker.getContainerInstance(containerId);
  const [metadata, currentContainer] = await Promise.all([
    container.inspect(),
    Docker.getCurrentContainerMetadata(),
  ]);
  if (metadata.Id === currentContainer.Id) {
    const error = new Error('Containarr cannot modify its own container.');
