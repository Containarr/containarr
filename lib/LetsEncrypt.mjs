...
const ACME_REQUEST_TIMEOUT = 1000 * 30;

acmeAxios.defaults.timeout = ACME_REQUEST_TIMEOUT;
...
export default class LetsEncrypt {
  ...
  constructor() {
    setAcmeLogger(message => this.debug(`[acme-client] ${message}`));
    ...
    this.#provisionQueue = this.#provisionQueue.catch(() => {}).then(async () => {
      ...
      const certificatePem = await client.auto({
        ...
        if (
          ...
          throw new Error('ACME request failed before receiving a response.');
        }
        throw error;
      });
      ...
      const retryAfterDate = Number.isSafeInteger(retryAfterSeconds) && retryAfterSeconds > 0
        ? Date.now() + retryAfterSeconds * 1000
        : new Date(retryAfter).getTime();
      const retryAt = new Date(Math.max(
        Date.now() + RETRY_INTERVAL,
        Number.isNaN(retryAfterDate) ? 0 : retryAfterDate,
      ));
      const errorMessage = error.response?.data?.detail ?? error.message;

      certificate.status = 'error';
      certificate.error = errorMessage;
      certificate.retryAt = retryAt;
      await certificate.save().catch(saveError => this.debug(saveError));
      this.debug(`Error Provisioning ${certificate.hostname}: ${errorMessage}; retry after ${retryAt.toISOString()}`);
    }).finally(() => {
