...
export default {
  ...
  backupVolumes: {
    ...
    validate: {
      validate(value) {
        const volumes = Array.isArray(this.dockerVolumes)
          ? this.dockerVolumes : Object.values(this.dockerVolumes ?? {});
        if (!Array.isArray(value) || value.some(volume => (
          typeof volume !== 'string' || !volumes.includes(volume)
          || !/^(?:\/[^:]+|[a-zA-Z0-9][a-zA-Z0-9_.-]*):\/[^:]+(?::[^:]+)?$/.test(volume)
          || /[\0\r\n]/.test(volume)
        ))) {
          throw new Error('Backup volumes must be selected from the app’s configured volumes.');
        }
      },
    },
  },
