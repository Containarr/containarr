import debug from 'debug';
import dockerode from 'dockerode';
import { randomUUID } from 'node:crypto';
import fs from 'node:fs/promises';
import { createWriteStream } from 'node:fs';
import { pipeline } from 'node:stream/promises';
import { createGzip } from 'node:zlib';
import os from 'node:os';
import path from 'node:path';

import {
  DOCKER_SOCK,
} from '../config.mjs';

export default class Docker {

  debug = debug('Docker');
  #currentContainerMetadata = null;

  constructor() {
    this.dockerode = Promise.resolve().then(async () => {
      const docker = new dockerode({
        socketPath: DOCKER_SOCK,
      });

      // Docker's local API must not redirect. docker-modem's redirect requests
      // lose the socket path and have no error listener (e.g. ENOTFOUND images).
      const buildRequest = docker.modem.buildRequest;
      docker.modem.buildRequest = function (options, ...args) {
        return buildRequest.call(this, { ...options, maxRedirects: 0 }, ...args);
      };

      await docker.ping();
      return docker;
    });

    this.dockerode
      .then(() => this.debug('Ready'))
      .catch(err => {
        this.debug(err);
        process.exit(1);
      });

    this.dockerode.then(async () => {
      const containers = await this.getContainers();
      this.debug(`Found ${containers.length} containers`);
    }).catch(error => this.debug(`Unable to List Containers: ${error.message}`));

    this.getCurrentContainerMetadata()
      .then(metadata => this.debug(`Current Container: ${metadata.Id}`))
      .catch(error => this.debug(`Unable to Identify Current Container: ${error.message}`));
  }

  async getCurrentContainerMetadata() {
    this.#currentContainerMetadata = this.#currentContainerMetadata || Promise.resolve().then(async () => {
      const docker = await this.dockerode;
      let containerId = null;

      try {
        const mountInfo = await fs.readFile('/proc/self/mountinfo', 'utf8');
        const containerMount = mountInfo.split('\n').find(line => line.includes('containers'));
        containerId = containerMount?.match(/\/containers\/([^/]+)\//)?.[1] ?? null;
      } catch {
        // Fall back to the configured container name.
      }

      if (containerId) {
        try {
          return await docker.getContainer(containerId).inspect();
        } catch {
          // Fall back to the configured container name.
        }
      }

      return docker.getContainer('containarr').inspect();
    }).catch(error => {
      this.#currentContainerMetadata = null;
      throw error;
    });

    return this.#currentContainerMetadata;
  }

  async archiveVolume({ volume, destination, backupDirectory }) {
    const [source, target] = volume.split(':');
    if (!source || !target?.startsWith('/') || /[\0\r\n]/.test(volume)
      || (!source.startsWith('/') && !/^[a-zA-Z0-9][a-zA-Z0-9_.-]*$/.test(source))) {
      throw new Error(`Invalid backup volume: ${volume}`);
    }
    const metadata = await this.getCurrentContainerMetadata();
    const docker = await this.dockerode;
    const type = source.startsWith('/') ? 'bind' : 'volume';
    // Never create an empty named volume when a selected source is missing.
    if (type === 'volume') await docker.getVolume(source).inspect();
    for (const mount of metadata.Mounts ?? []) {
      if (backupDirectory !== mount.Destination && !backupDirectory.startsWith(`${mount.Destination}/`)) continue;
      const backupSource = path.posix.join(mount.Source, path.posix.relative(mount.Destination, backupDirectory));
      if ((type === 'volume' && source === mount.Name)
        || (type === 'bind' && (backupSource === path.posix.resolve(source)
          || backupSource.startsWith(`${path.posix.resolve(source).replace(/\/$/, '')}/`)))) {
        throw new Error('A selected volume contains Containarr’s backup directory. Exclude it to avoid backing up backups.');
      }
    }
    const container = await docker.createContainer({
      name: `containarr-backup-volume-${randomUUID()}`,
      Image: metadata.Image,
      Entrypoint: ['/bin/sleep'],
      Cmd: ['infinity'],
      User: '0:0',
      Labels: { 'containarr.helper': 'backup-volume' },
      HostConfig: {
        NetworkMode: 'none',
        ReadonlyRootfs: true,
        Mounts: [{
          Type: type,
          Source: source,
          Target: '/backup-source',
          ReadOnly: true,
          ...(type === 'volume' ? { VolumeOptions: { NoCopy: true } } : {}),
        }],
      },
    });
    try {
      await container.start();
      const archive = await container.getArchive({ path: '/backup-source' });
      await pipeline(archive, createGzip(), createWriteStream(destination, { mode: 0o600 }));
    } finally {
      await container.remove({ force: true, v: true });
    }
  }

  async getHostIpAddresses() {
    let metadata;
    try {
      metadata = await this.getCurrentContainerMetadata();
    } catch {
      return Object.values(os.networkInterfaces())
        .flat()
        .filter(address => address?.family === 'IPv4' && !address.internal)
        .map(address => address.address);
    }

    const docker = await this.dockerode;
    const container = await docker.createContainer({
      name: `containarr-list-host-ip-addresses-${randomUUID()}`,
      Image: metadata.Image,
      Entrypoint: ['/usr/local/bin/node'],
      Cmd: ['/app/lib/Docker/ListHostIpAddresses.mjs'],
      Labels: {
        'containarr.helper': 'list-host-ip-addresses',
      },
      Tty: false,
      HostConfig: {
        NetworkMode: 'host',
      },
    });

    try {
      await container.start();
      const result = await container.wait();
      const output = decodeDockerLogStream(Buffer.from(
        await container.logs({ stdout: true, stderr: true }),
      ));
      if (result.StatusCode !== 0) {
        throw new Error(output.trim() || 'Unable to inspect host IP addresses.');
      }
      const addresses = JSON.parse(output);
      if (!Array.isArray(addresses) || addresses.some(address => typeof address !== 'string')) {
        throw new Error('Host IP inspection returned an invalid response.');
      }
      return addresses;
    } finally {
      await container.remove({ force: true }).catch(() => {});
    }
  }

  /*
   * Images
   */
  validateImage(image) {
    // Docker references are registry/repository[:tag][@digest], without a URL
    // scheme. Grammar: https://github.com/distribution/reference/blob/main/regexp.go
    const component = '[a-z0-9]+(?:(?:[._]|__|-+)[a-z0-9]+)*';
    const domain = '(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?)(?:\\.[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?)*';
    const reference = new RegExp(`^((?:(?:${domain}|\\[[a-fA-F0-9:]+\\])(?::[0-9]+)?/)?${component}(?:/${component})*)(?::[\\w][\\w.-]{0,127})?(?:@(sha256:[a-f0-9]{64}|sha384:[a-f0-9]{96}|sha512:[a-f0-9]{128}))?$`);
    const match = typeof image === 'string' && image.length <= 1024 && !/\s/.test(image) && reference.exec(image);
    if (
      !match
      || match[1].length > 255
      || (image.startsWith('sha256:') && !/^sha256:[a-f0-9]{64}$/.test(image))
    ) {
      throw Object.assign(new Error('Enter a valid Docker image ID or registry/repository[:tag][@digest], for example ghcr.io/example/app:latest. Do not include http:// or https://.'), { statusCode: 400 });
    }
  }

  async getImages() {
    const docker = await this.dockerode;
    return docker.listImages();
  }

  async cleanupImages() {
    const docker = await this.dockerode;
    return docker.pruneImages({
      filters: {
        dangling: ['false'],
      },
    });
  }

  async deleteImage({
    imageId,
  }) {
    this.validateImage(imageId);
    const docker = await this.dockerode;
    return docker.getImage(imageId).remove();
  }

  async deleteImageIfUnused({
    imageId,
  }) {
    if (!imageId) return false;
    this.validateImage(imageId);

    const containers = await this.getContainers();
    if (containers.some(container => container.ImageID === imageId)) return false;

    const docker = await this.dockerode;
    try {
      await docker.getImage(imageId).remove({ force: true });
      return true;
    } catch (error) {
      if ([404, 409].includes(error.statusCode)) return false;
      throw error;
    }
  }

  async pullImage(image) {
    this.validateImage(image);
    const docker = await this.dockerode;
    const stream = await docker.pull(image);
    const output = [];
    let pending = '';
    // Parse inside the async operation so malformed responses and stream errors
    // reject the pull instead of throwing from an EventEmitter callback.
    stream.setEncoding('utf8');
    for await (const chunk of stream) {
      pending += chunk;
      let separator;
      while ((separator = pending.indexOf('\n')) !== -1) {
        const line = pending.slice(0, separator).trim();
        pending = pending.slice(separator + 1);
        if (!line) continue;
        const progress = JSON.parse(line);
        if (progress.error || progress.errorDetail?.message) {
          throw new Error(progress.errorDetail?.message || progress.error);
        }
        output.push(progress);
      }
    }
    if (pending.trim()) {
      const progress = JSON.parse(pending);
      if (progress.error || progress.errorDetail?.message) {
        throw new Error(progress.errorDetail?.message || progress.error);
      }
      output.push(progress);
    }
    return output;
  }

  async getImageMetadata(image) {
    this.validateImage(image);
    const docker = await this.dockerode;
    return docker.getImage(image).inspect();
  }

  async getPathSuggestions({ source, requestedPath, image = null }) {
    if (!['host', 'device', 'image'].includes(source)) {
      throw new Error(`Invalid path source: ${source}`);
    }
    if (typeof requestedPath !== 'string' || !requestedPath.startsWith('/') || requestedPath.includes('\0')) {
      throw new Error('Path must be absolute.');
    }
    if (source === 'image' && !image) {
      throw new Error('Missing Docker image.');
    }

    const docker = await this.dockerode;
    let container;

    try {
      if (source === 'image') {
        await this.getImageMetadata(image).catch(error => {
          if (error.statusCode === 404) return this.pullImage(image);
          throw error;
        });
        const separator = requestedPath.endsWith('/')
          ? requestedPath.length - 1
          : requestedPath.lastIndexOf('/');
        const directory = requestedPath.endsWith('/')
          ? requestedPath.replace(/\/+$/, '') || '/'
          : requestedPath.slice(0, separator) || '/';
        const prefix = requestedPath.endsWith('/')
          ? ''
          : requestedPath.slice(separator + 1);

        container = await docker.createContainer({
          name: `containarr-list-image-paths-${randomUUID()}`,
          Image: image,
          Entrypoint: ['ls'],
          Cmd: ['-1Ap', directory],
          Labels: {
            'containarr.helper': 'list-image-paths',
          },
          Tty: false,
          HostConfig: {
            NetworkMode: 'none',
          },
        });
        await container.start();
        let waitTimeout;
        const result = await Promise.race([
          container.wait(),
          new Promise((resolve, reject) => {
            waitTimeout = setTimeout(() => reject(new Error(`Timed out inspecting ${image}.`)), 5000);
          }),
        ]).finally(() => clearTimeout(waitTimeout));
        const output = decodeDockerLogStream(Buffer.from(
          await container.logs({ stdout: true, stderr: true }),
        ));
        if (result.StatusCode !== 0) {
          throw new Error(output.trim() || `Unable to inspect ${image}.`);
        }

        return output
          .split('\n')
          .map(entry => entry.trim())
          .filter(entry => entry && !['./', '../'].includes(entry))
          .filter(entry => entry.replace(/\/$/, '').toLowerCase().startsWith(prefix.toLowerCase()))
          .sort((left, right) => (
            Number(right.endsWith('/')) - Number(left.endsWith('/'))
            || left.localeCompare(right)
          ))
          .slice(0, 100)
          .map(entry => ({
            path: `${directory === '/' ? '' : directory}/${entry}`,
            directory: entry.endsWith('/'),
          }));
      }

      const publicRoot = source === 'device' ? '/dev' : '/';
      if (publicRoot !== '/' && requestedPath !== publicRoot && !requestedPath.startsWith(`${publicRoot}/`)) {
        throw new Error('Path is outside the requested root.');
      }
      const relative = publicRoot === '/'
        ? requestedPath
        : requestedPath.slice(publicRoot.length) || '/';
      const separator = relative.endsWith('/')
        ? relative.length - 1
        : relative.lastIndexOf('/');
      const directory = relative.endsWith('/')
        ? relative.slice(0, -1) || '/'
        : relative.slice(0, separator) || '/';
      const prefix = relative.endsWith('/') ? '' : relative.slice(separator + 1);
      const resolved = path.posix.resolve('/host', `.${directory}`);
      if (resolved !== '/host' && !resolved.startsWith('/host/')) {
        throw new Error('Path is outside the mounted root.');
      }

      const metadata = await this.getCurrentContainerMetadata();

      container = await docker.createContainer({
        name: `containarr-list-host-paths-${randomUUID()}`,
        Image: metadata.Image,
        Entrypoint: ['/bin/ls'],
        Cmd: [
          // The Containarr image provides GNU ls. NUL separators preserve
          // whitespace and newlines in filenames; sorting happens below.
          '-1ApU', '--zero', '--color=never', '--quoting-style=literal', '--', `${resolved}/`,
        ],
        Labels: {
          'containarr.helper': 'list-host-paths',
        },
        Tty: false,
        HostConfig: {
          Binds: [`${source === 'device' ? '/dev' : '/'}:/host:ro`],
          NetworkMode: 'none',
        },
      });
      await container.start();
      let waitTimeout;
      const result = await Promise.race([
        container.wait(),
        new Promise((resolve, reject) => {
          waitTimeout = setTimeout(() => reject(new Error('Timed out inspecting host paths.')), 5000);
        }),
      ]).finally(() => clearTimeout(waitTimeout));
      const output = decodeDockerLogStream(Buffer.from(
        await container.logs({ stdout: true, stderr: true }),
      ));
      if (result.StatusCode !== 0) {
        throw new Error(output.trim() || 'Unable to inspect host paths.');
      }
      return output
        .split('\0')
        .filter(entry => entry && entry.replace(/\/$/, '').toLowerCase().startsWith(prefix.toLowerCase()))
        .sort((left, right) => (
          Number(right.endsWith('/')) - Number(left.endsWith('/'))
          || left.replace(/\/$/, '').localeCompare(right.replace(/\/$/, ''))
        ))
        .slice(0, 100)
        .map(entry => ({
          path: `${publicRoot === '/' ? '' : publicRoot}${directory === '/' ? '' : directory}/${entry}`,
          directory: entry.endsWith('/'),
        }));
    } finally {
      if (container) {
        await container.remove({ force: true }).catch(() => {});
      }
    }
  }

  async createHostDirectory({ parentPath, directoryName }) {
    if (typeof parentPath !== 'string' || !parentPath.startsWith('/')) {
      throw Object.assign(new Error('Parent path must be absolute.'), { statusCode: 400 });
    }
    const normalizedDirectoryName = typeof directoryName === 'string'
      ? directoryName.trim()
      : '';
    if (
      !normalizedDirectoryName
      || ['.', '..'].includes(normalizedDirectoryName)
      || normalizedDirectoryName.includes('/')
      || normalizedDirectoryName.includes('\0')
    ) {
      throw Object.assign(new Error('Enter a valid folder name.'), { statusCode: 400 });
    }

    const docker = await this.dockerode;
    const metadata = await this.getCurrentContainerMetadata();
    let container;

    try {
      container = await docker.createContainer({
        name: `containarr-create-folder-${randomUUID()}`,
        Image: metadata.Image,
        Entrypoint: ['/usr/local/bin/node'],
        Cmd: [
          '/app/lib/Docker/CreateDirectory.mjs',
          parentPath,
          normalizedDirectoryName,
        ],
        Labels: {
          'containarr.helper': 'create-folder',
        },
        Tty: false,
        HostConfig: {
          Binds: ['/:/host'],
          NetworkMode: 'none',
        },
      });
      await container.start();
      let waitTimeout;
      const result = await Promise.race([
        container.wait(),
        new Promise((resolve, reject) => {
          waitTimeout = setTimeout(() => reject(new Error('Timed out creating the host folder.')), 5000);
        }),
      ]).finally(() => clearTimeout(waitTimeout));
      const output = decodeDockerLogStream(Buffer.from(
        await container.logs({ stdout: true, stderr: true }),
      ));
      if (result.StatusCode !== 0) {
        throw new Error(output.trim() || 'Unable to create the host folder.');
      }
      return JSON.parse(output);
    } finally {
      if (container) {
        await container.remove({ force: true }).catch(() => {});
      }
    }
  }

  /*
   * Containers
   */
  async getContainers() {
    const docker = await this.dockerode;
    return docker.listContainers({
      all: true,
    });
  }

  async cleanupContainers() {
    const docker = await this.dockerode;
    return docker.pruneContainers();
  }

  /*
   * Volumes
   */
  async calculateVolumeSize({ volume }) {
    const [source, target] = typeof volume === 'string' ? volume.split(':') : [];
    if (!source || !target?.startsWith('/') || /[\0\r\n]/.test(volume)
      || (!source.startsWith('/') && !/^[a-zA-Z0-9][a-zA-Z0-9_.-]*$/.test(source))) {
      throw Object.assign(new Error('Invalid volume.'), { statusCode: 400 });
    }
    const docker = await this.dockerode;
    const type = source.startsWith('/') ? 'bind' : 'volume';
    // Inspect first so a missing named volume is never created by this request.
    if (type === 'volume') await docker.getVolume(source).inspect();
    const metadata = await this.getCurrentContainerMetadata();
    const container = await docker.createContainer({
      name: `containarr-volume-size-${randomUUID()}`,
      Image: metadata.Image,
      Entrypoint: ['/usr/bin/du'],
      Cmd: ['-sb', '--', '/volume-source'],
      User: '0:0',
      Labels: { 'containarr.helper': 'volume-size' },
      Tty: false,
      HostConfig: {
        NetworkMode: 'none',
        ReadonlyRootfs: true,
        Mounts: [{
          Type: type,
          Source: source,
          Target: '/volume-source',
          ReadOnly: true,
          ...(type === 'volume' ? { VolumeOptions: { NoCopy: true } } : {}),
        }],
      },
    });
    try {
      await container.start();
      let waitTimeout;
      const result = await Promise.race([
        container.wait(),
        new Promise((resolve, reject) => {
          waitTimeout = setTimeout(() => reject(new Error('Timed out calculating volume size.')), 60000);
        }),
      ]).finally(() => clearTimeout(waitTimeout));
      const output = decodeDockerLogStream(Buffer.from(
        await container.logs({ stdout: true, stderr: true }),
      ));
      if (result.StatusCode !== 0) {
        throw new Error(output.trim() || 'Unable to calculate volume size.');
      }
      const match = output.match(/^(\d+)\s+\/volume-source\s*$/);
      const size = match ? Number(match[1]) : NaN;
      if (!Number.isSafeInteger(size) || size < 0) {
        throw new Error('Volume size calculation returned an invalid response.');
      }
      return size;
    } finally {
      await container.remove({ force: true, v: true });
    }
  }

  async getVolumes() {
    const docker = await this.dockerode;
    const [result, diskUsage] = await Promise.all([
      docker.listVolumes(),
      docker.df(),
    ]);
    const usageByName = new Map(
      (diskUsage.Volumes ?? []).map(volume => [volume.Name, volume.UsageData]),
    );
    return {
      ...result,
      Volumes: (result.Volumes ?? []).map(volume => ({
        ...volume,
        UsageData: usageByName.get(volume.Name) ?? volume.UsageData,
      })),
    };
  }

  async cleanupVolumes() {
    const docker = await this.dockerode;
    return docker.pruneVolumes({
      filters: {
        all: ['true'],
      },
    });
  }

  async deleteVolume({
    volumeName,
  }) {
    const docker = await this.dockerode;
    return docker.getVolume(volumeName).remove();
  }

  async deleteVolumeIfUnused({
    volumeName,
  }) {
    if (!volumeName) return false;

    const containers = await this.getContainers();
    if (containers.some(container => (container.Mounts ?? []).some(mount => (
      mount.Type === 'volume'
      && (mount.Name === volumeName || mount.Source === volumeName)
    )))) return false;

    const docker = await this.dockerode;
    try {
      await docker.getVolume(volumeName).remove();
      return true;
    } catch (error) {
      if ([404, 409].includes(error.statusCode)) return false;
      throw error;
    }
  }

  /*
   * Networks
   */
  async getNetworks() {
    const docker = await this.dockerode;
    const networks = await docker.listNetworks();
    return Promise.all(networks.map(async network => {
      try {
        return await docker.getNetwork(network.Id).inspect();
      } catch {
        return network;
      }
    }));
  }

  async createNetwork({
    name,
  }) {
    const docker = await this.dockerode;
    const network = await docker.createNetwork({
      Name: name,
      Driver: 'bridge',
    });
    return network.inspect();
  }

  async cleanupNetworks() {
    const docker = await this.dockerode;
    return docker.pruneNetworks();
  }

  async deleteNetwork({
    networkId,
  }) {
    const docker = await this.dockerode;
    const network = docker.getNetwork(networkId);
    const metadata = await network.inspect();
    if (Object.keys(metadata.Containers ?? {}).length > 0) {
      throw new Error(`Network ${metadata.Name} has active endpoints and cannot be deleted.`);
    }
    return network.remove();
  }

  async deleteNetworkIfUnused({
    networkId,
  }) {
    if (!networkId) return false;

    const docker = await this.dockerode;
    const network = docker.getNetwork(networkId);
    try {
      const metadata = await network.inspect();
      if (
        Object.keys(metadata.Containers ?? {}).length > 0
        || metadata.Ingress
        || ['bridge', 'host', 'none'].includes(metadata.Name)
      ) return false;

      await network.remove();
      return true;
    } catch (error) {
      if ([404, 409].includes(error.statusCode)) return false;
      throw error;
    }
  }

  async findContainerByLabel({ label, value }) {
    const containers = await this.getContainers();
    const container = containers.find(container => container.Labels && container.Labels[label] === value);
    if (!container) return null;

    const docker = await this.dockerode;
    return container;
  }

  async getContainerInstance(containerId) {
    const docker = await this.dockerode;
    return docker.getContainer(containerId);
  }

  async getContainerLogs({
    containerId,
    tail = 200,
  }) {
    const container = await this.getContainerInstance(containerId);
    const [metadata, output] = await Promise.all([
      container.inspect(),
      container.logs({
        stdout: true,
        stderr: true,
        timestamps: true,
        tail,
      }),
    ]);
    const buffer = Buffer.isBuffer(output) ? output : Buffer.from(output);

    if (metadata.Config.Tty) return buffer.toString('utf8');
    return decodeDockerLogStream(buffer);
  }

  async getContainerStats({
    containerId,
  }) {
    const container = await this.getContainerInstance(containerId);
    const metadata = await container.inspect();

    if (!metadata.State.Running) {
      return {
        id: metadata.Id,
        read: new Date().toISOString(),
        cpuPercent: 0,
        memoryUsage: 0,
        memoryLimit: 0,
        blockReadBytes: 0,
        blockWriteBytes: 0,
        networkRxBytes: 0,
        networkTxBytes: 0,
      };
    }

    const stats = await container.stats({ stream: false });
    const cpuUsage = stats.cpu_stats?.cpu_usage ?? {};
    const previousCpuUsage = stats.precpu_stats?.cpu_usage ?? {};
    const cpuDelta = (cpuUsage.total_usage ?? 0) - (previousCpuUsage.total_usage ?? 0);
    const systemDelta = (stats.cpu_stats?.system_cpu_usage ?? 0)
      - (stats.precpu_stats?.system_cpu_usage ?? 0);
    const onlineCpus = stats.cpu_stats?.online_cpus
      ?? cpuUsage.percpu_usage?.length
      ?? 1;
    const cpuPercent = cpuDelta > 0 && systemDelta > 0
      ? (cpuDelta / systemDelta) * onlineCpus * 100
      : 0;

    const memoryStats = stats.memory_stats ?? {};
    const cache = memoryStats.stats?.inactive_file
      ?? memoryStats.stats?.total_inactive_file
      ?? memoryStats.stats?.cache
      ?? 0;
    const memoryUsage = Math.max(0, (memoryStats.usage ?? 0) - cache);

    let blockReadBytes = 0;
    let blockWriteBytes = 0;
    for (const entry of stats.blkio_stats?.io_service_bytes_recursive ?? []) {
      const operation = entry.op?.toLowerCase();
      if (operation === 'read') blockReadBytes += entry.value ?? 0;
      if (operation === 'write') blockWriteBytes += entry.value ?? 0;
    }

    let networkRxBytes = 0;
    let networkTxBytes = 0;
    for (const network of Object.values(stats.networks ?? {})) {
      networkRxBytes += network.rx_bytes ?? 0;
      networkTxBytes += network.tx_bytes ?? 0;
    }

    return {
      id: metadata.Id,
      read: stats.read ?? new Date().toISOString(),
      cpuPercent: Number.isFinite(cpuPercent) ? cpuPercent : 0,
      memoryUsage: Number.isFinite(memoryUsage) ? memoryUsage : 0,
      memoryLimit: Number.isFinite(memoryStats.limit) ? memoryStats.limit : 0,
      blockReadBytes: Number.isFinite(blockReadBytes) ? blockReadBytes : 0,
      blockWriteBytes: Number.isFinite(blockWriteBytes) ? blockWriteBytes : 0,
      networkRxBytes: Number.isFinite(networkRxBytes) ? networkRxBytes : 0,
      networkTxBytes: Number.isFinite(networkTxBytes) ? networkTxBytes : 0,
    };
  }

  async createContainerShell({
    containerId,
    onClose,
    onData,
    onError,
  }) {
    const container = await this.getContainerInstance(containerId);
    const metadata = await container.inspect();
    if (!metadata.State.Running) {
      throw new Error('The container must be running to open a shell.');
    }

    const exec = await container.exec({
      AttachStderr: true,
      AttachStdin: true,
      AttachStdout: true,
      Cmd: ['/bin/sh'],
      Env: ['TERM=xterm-256color'],
      Tty: true,
    });
    const stream = await exec.start({
      hijack: true,
      stdin: true,
      Tty: true,
    });

    stream.on('data', data => onData(data.toString('utf8')));
    stream.on('end', onClose);
    stream.on('close', onClose);
    stream.on('error', onError);

    return {
      close() {
        stream.end();
        stream.destroy();
      },
      resize({ columns, rows }) {
        return exec.resize({
          h: rows,
          w: columns,
        });
      },
      write(data) {
        stream.write(data);
      },
    };
  }

  async createContainer({
    name = null,
    labels = {},
    image = null,
    command = null,
    entrypoint = null,
    workingDirectory = null,
    networkMode = 'bridge',
    networks = [],
    volumes = [],
    devices = [],
    ports = [],
    environment = {},
    user = null,
    userId = null,
    groupId = null,
    privileged = false,
    capabilities = [],
  }) {
    this.validateImage(image);
    const docker = await this.dockerode;
    if (!['bridge', 'host'].includes(networkMode)) {
      throw new Error(`Invalid network mode: ${networkMode}`);
    }

    const primaryNetwork = networkMode === 'bridge'
      ? networks.find(network => network?.name)
      : null;
    const portBindings = ports.reduce((acc, port) => {
      const key = `${port.container}/${port.protocol}`;
      acc[key] = acc[key] ?? [];
      acc[key].push({
        HostIp: port.hostIp || '',
        HostPort: `${port.host}`,
      });
      return acc;
    }, {});
    const containerConfig = {
      name,
      Labels: labels,
      Image: image,
      Cmd: command,
      Entrypoint: entrypoint,
      WorkingDir: workingDirectory || undefined,
      HostConfig: {
        Binds: volumes,
        Devices: devices.map(device => {
          const [pathOnHost, pathInContainer = pathOnHost, cgroupPermissions = 'rwm'] = device.split(':');
          return {
            PathOnHost: pathOnHost,
            PathInContainer: pathInContainer,
            CgroupPermissions: cgroupPermissions,
          };
        }),
        NetworkMode: networkMode === 'host' ? 'host' : primaryNetwork?.name,
        PortBindings: networkMode === 'host' ? {} : portBindings,
        RestartPolicy: {
          Name: 'unless-stopped',
          MaximumRetryCount: 0,
        },
        Privileged: privileged,
        CapAdd: capabilities,
      },
      Env: Object.entries(environment).map(([key, value]) => `${key}=${value}`),
      User: userId === null
        ? user || undefined
        : groupId === null
          ? `${userId}`
          : `${userId}:${groupId}`,
      NetworkingConfig: primaryNetwork ? {
        EndpointsConfig: {
          [primaryNetwork.name]: {
            Aliases: Array.isArray(primaryNetwork.aliases) ? primaryNetwork.aliases : [],
          },
        },
      } : undefined,
    };

    const container = await docker.createContainer(containerConfig);

    try {
      if (networkMode !== 'host') {
        for (const network of networks) {
          if (!network?.name) continue;
          if (network.name === primaryNetwork?.name) continue;
          await docker.getNetwork(network.name).connect({
            Container: container.id,
            EndpointConfig: {
              Aliases: Array.isArray(network.aliases) ? network.aliases : [],
            },
          });
        }
      }
    } catch (error) {
      await container.remove({ force: true }).catch(() => {});
      throw error;
    }

    return container;
  }

  async deleteContainer({
    containerId,
  }) {
    const docker = await this.dockerode;
    const container = docker.getContainer(containerId);
    await container.remove({
      force: true,
    });
  }
}

function decodeDockerLogStream(buffer) {
  let offset = 0;
  let output = '';

  while (offset + 8 <= buffer.length) {
    const length = buffer.readUInt32BE(offset + 4);
    const start = offset + 8;
    const end = start + length;
    if (end > buffer.length) break;

    output += buffer.subarray(start, end).toString('utf8');
    offset = end;
  }

  return output || buffer.toString('utf8');
}
