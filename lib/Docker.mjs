...
const relative = publicRoot === '/' ? requested : requested.slice(publicRoot.length) || '/';
const separator = relative.endsWith('/') ? relative.length - 1 : relative.lastIndexOf('/');
const directory = relative.endsWith('/') ? relative.slice(0, -1) || '/' : relative.slice(0, separator) || '/';
