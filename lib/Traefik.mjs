...
import HTTPServer from '../services/HTTPServer.mjs';
  ...
  constructor() {
    this.#process = Promise.resolve().then(async () => {
      ...
      const traefik = child_process.spawn('traefik', [
        ...
        '--accesslog.fields.headers.names.X-Containarr-Client=keep',
        '--accesslog.fields.queryparameters.defaultmode=drop',
        ...['StartUTC', 'RequestMethod', 'RequestHost', 'RequestPath', 'ClientHost', 'DownstreamStatus', 'Duration', 'DownstreamContentSize', 'ServiceName']
          .map(field => `--accesslog.fields.names.${field}=keep`),
      ...
      readline.createInterface({
        ...
        if (!Traffic.record(line)) this.debug(`[stdout] ${line}`);
      });
      ...
      traefik.once('close', async (code) => {
        this.debug(`Traefik exited with code ${code}`);
        clearInterval(Traffic.flushInterval);
        try {
          await Traffic.operation;
          await Traffic.flush();
        } catch (error) {
          this.debug('Could not save traffic logs during shutdown:', error);
        }
  ...
  async getConfig() {
    ...
    for (const app of Object.values(apps)) {
      ...
      config.http.services[id] = {
        loadBalancer: {
          servers: [{
            url: app.dockerNetworkMode === 'host' && app.port
      ...
      switch (app.tls || 'only_https') {
        case 'only_http':
          config.http.routers[`${id}-http`] = {
            ...
            entryPoints: ['http'],
            ...(app.policyId === 'public' ? {} : {
              middlewares: [`firewall-${app.policyId}`],
            }),
          };
          break;
        case 'only_https':
        ...
        case 'only_http':
          config.http.routers[`${id}-http`] = {
            ...
            ...(proxy.policyId === 'public' ? {} : {
              middlewares: [`firewall-${proxy.policyId}`],
            }),
          };
          break;
        case 'only_https':
