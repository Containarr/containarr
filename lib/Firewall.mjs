...
export default class Firewall {
  ...
  async createPolicy({ name, allowedIps = [], tailscaleDevices = [] }) {
    await this.ready;
    const Policy = await SQLite.getModelPolicy();
    return Policy.create({ name, allowedIps, tailscaleDevices });
  }

  async updatePolicy({ policyId, name, allowedIps, tailscaleDevices }) {
    if (policyId === 'public') {
      throw new Error('The Public policy cannot be changed.');
    }
    const policy = await this.getPolicy({ policyId });
    policy.set({ name, allowedIps, tailscaleDevices });
    await policy.save();
    return policy;
  }
  ...
  async isAllowed({ policyId, address }) {
    ...
    for (const entry of [
      ...policy.allowedIps,
      ...(policy.tailscaleDevices ?? []).map(device => device.address),
    ]) {
