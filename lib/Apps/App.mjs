...
export default class App {
  ...
  async sync() {
    ...
    try {
      ...
      if (Date.now() - this.#lastEventCheck >= 15_000) {
        this.#lastEventCheck = Date.now();
        await Events.observeApp({
          appId: this.id,
          appName: this.name,
          online: this.state?.toLowerCase() === 'running'
            && !/\(unhealthy\)|\(health: starting\)/i.test(this.#containerMetadata?.Status ?? ''),
          disabled: this.disabled,
        }).catch(error => this.debug(`Error Recording App Status: ${error.message}`));
      }
  ...
  async createContainer() {
    ...
    await Docker.getImageMetadata(this.db.dockerImage).catch(error => {
      if (error.statusCode === 404) return Docker.pullImage(this.db.dockerImage);
      throw error;
    });
  ...
  async importContainer({
    ...
    try {
      ...
      this.db.containerError = {
        ...
        logs: '',
        message: `Unable to import the container: ${error.message}`,
      };
      await this.db.save();
      throw error;
    } finally {
  ...
  async checkImageUpdate({ applyUpdate = this.autoUpdate } = {}) {
    this.#checkUpdatePromise = this.#checkUpdatePromise || Promise.resolve().then(async () => {
      ...
      try {
        ...
        if (updateAvailable && !this.autoUpdate) {
          await Events.record({
            eventName: 'app.update_available', message: `${this.name} has an update available.`,
            appId: this.id, appName: this.name,
            details: { dockerImage: this.db.dockerImage, imageId: image.Id },
          }, { key: `events.app-update.${this.id}`, value: image.Id })
            .catch(error => this.debug(`Error Recording Available Update: ${error.message}`));
        }
        ...
        if (updateAvailable && applyUpdate) {
          ...
          await Events.record({
            eventName: 'app.updated', message: `${this.name} has been updated.`,
            appId: this.id, appName: this.name,
            details: { dockerImage: this.db.dockerImage, imageId: image.Id },
          }).catch(error => this.debug(`Error Recording App Update: ${error.message}`));
        }
  ...
  toJSON() {
    ...
    return {
      ...
      logoVersion: this.hasLogo ? createHash('sha256').update(this.db.logo).digest('hex') : null,
      tls: this.tls,
