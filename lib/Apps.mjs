...
export default class Apps {
  ...
  async importContainer({ containerId, settings = null, preview = false }) {
    ...
    if (process.env.HOSTNAME && metadata.Id.startsWith(process.env.HOSTNAME)) {
    ...
    await Docker.getImageMetadata(configuration.dockerImage)
      .catch(() => Docker.pullImage(configuration.dockerImage));
    const appDatabase = await AppDatabase.create(configuration);

    const wasRunning = Boolean(metadata.State?.Running);
    const temporaryName = `containarr-import-${metadata.Id.slice(0, 12)}`;
    let replacement = null;
    try {
      if (wasRunning) await container.stop();
      await container.rename({ name: temporaryName });
      replacement = await Docker.createContainer({
        name: configuration.subdomain,
        labels: {
          'containarr.app.id': appDatabase.id,
        },
        image: configuration.dockerImage,
        command: configuration.dockerCommand,
        entrypoint: configuration.dockerEntrypoint,
        workingDirectory: configuration.dockerWorkingDirectory,
        networkMode: configuration.dockerNetworkMode,
        networks: configuration.dockerNetworks,
        volumes: configuration.dockerVolumes,
        devices: configuration.dockerDevices,
        ports: configuration.dockerPorts,
        environment: configuration.dockerEnvironment,
        user: configuration.dockerUser,
        userId: configuration.dockerUserId,
        groupId: configuration.dockerGroupId,
        privileged: configuration.dockerPrivileged,
        capabilities: configuration.dockerCapabilities,
      });
      await replacement.start();
      await container.remove({ force: true });
    } catch (error) {
      if (replacement) await replacement.remove({ force: true }).catch(() => {});
      await container.rename({ name: originalName }).catch(() => {});
      if (wasRunning) await container.start().catch(() => {});
      await appDatabase.destroy();
      throw error;
    }

    const appInstance = new App({ db: appDatabase });
    this.#apps.set(appDatabase.id, appInstance);
    return appInstance;
  }
  ...
  async deleteApp({ appId }) {
    ...
    const networkIds = new Set(
      containers.flatMap(container => Object.entries(container.NetworkSettings?.Networks ?? {})
        .filter(([networkName]) => !['bridge', 'host', 'none'].includes(networkName))
        .map(([networkName, network]) => network.NetworkID || networkName)),
    );
    ...
    for (const network of appInstance.db.dockerNetworks ?? []) {
      if (network?.name && !['bridge', 'host', 'none'].includes(network.name)) {
        networkIds.add(network.name);
      }
    }
    ...
    for (const networkId of networkIds) {
      await Docker.deleteNetworkIfUnused({ networkId }).catch(error => {
        appInstance.debug(`Error Cleaning Up Deleted App Network: ${error.message}`);
      });
    }
