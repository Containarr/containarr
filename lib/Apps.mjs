...
export default class Apps {
  ...
  async createApp({
    ...
    Docker.validateImage(dockerImage);
    await this.#ready;
  ...
  async importContainer({ containerId, settings = null, preview = false }) {
    ...
    const configuration = {
      ...
      ...(settings ? {
        ...
        backupVolumes: settings.backupVolumes ?? [],
    ...
    Docker.validateImage(configuration.dockerImage);
  ...
  async updateApp({
    ...
    appInstance.db.set({
      ...
      dockerVolumes,
      backupVolumes: backupVolumes ?? (appInstance.db.backupVolumes ?? []).filter(volume => (
        (dockerVolumes ?? appInstance.db.dockerVolumes ?? []).includes(volume)
      )),
      dockerDevices,
      dockerPorts,
      dockerEnvironment,
  ...
  async updateAppLogo({ appId, logo }) {
    if (logo !== null) {
      if (typeof logo !== 'string' || logo.length > 512 * 1024
        || !/^data:image\/png;base64,(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(logo)) {
        throw Object.assign(new Error('Upload a PNG icon up to 384 KB.'), { statusCode: 400 });
      }
      logo = Buffer.from(logo.slice('data:image/png;base64,'.length), 'base64');
      if (logo.length < 45
        || !logo.subarray(0, 8).equals(Buffer.from('89504e470d0a1a0a', 'hex'))
        || logo.readUInt32BE(8) !== 13 || logo.toString('ascii', 12, 16) !== 'IHDR'
        || logo.readUInt32BE(16) < 1 || logo.readUInt32BE(16) > 256
        || logo.readUInt32BE(20) < 1 || logo.readUInt32BE(20) > 256
        || !logo.subarray(-12).equals(Buffer.from('0000000049454e44ae426082', 'hex'))) {
        throw Object.assign(new Error('Upload a valid PNG icon no larger than 256 × 256 pixels.'), { statusCode: 400 });
      }
    }
    const appInstance = await this.getApp({ appId });
    appInstance.db.set({ logo });
    await appInstance.db.save();
    return appInstance;
  }
