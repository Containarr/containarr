...
export default class Updates {
    ...
    this.#checkPromise = this.#checkPromise || Promise.resolve().then(async () => {
      try {
        ...
        if (this.#status.updateAvailable) {
          await Events.record({
            eventName: 'containarr.update_available',
            message: `Containarr has an update available: v${latestVersion}.`,
            details: { version: latestVersion, currentVersion },
          }, { key: 'events.containarr-update', value: latestVersion })
  ...
  async install() {
    this.#installPromise = this.#installPromise || Promise.resolve().then(async () => {
      ...
      try {
        this.debug(`Installing ${CONTAINARR_IMAGE}`);
        const docker = await Docker.dockerode;
        const metadata = await Docker.getCurrentContainerMetadata();
        this.debug(`Inspecting current container ${metadata.Id}`);
        this.debug(`Pulling ${CONTAINARR_IMAGE}`);
        await Docker.pullImage(CONTAINARR_IMAGE);
        this.debug(`Pulled ${CONTAINARR_IMAGE}`);

        this.debug('Creating updater helper container');
        const helper = await docker.createContainer({
          name: `containarr-updater-${randomUUID()}`,
        ...
        this.debug(`Starting updater helper ${helper.id}`);
        await helper.start();
        helper.logs({
          follow: true,
          stdout: true,
          ...
          logs.on('data', data => {
            ...
            if (message) this.debug(`Helper: ${message}`);
          });
          logs.on('error', error => this.debug(`Helper logs: ${error.message}`));
        }).catch(error => this.debug(`Helper logs: ${error.message}`));
        this.debug(`Updater helper ${helper.id} started`);
      } catch (error) {
        this.debug(`Install failed: ${error.stack || error.message}`);
